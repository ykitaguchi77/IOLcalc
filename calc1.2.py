   ...
   def save_extracted_numbers(self):
        ...
        if targets:
            ...
            if id_number in df['整理番号'].values:
                ...
                #numbers[0]はIOL power
                df.loc[index, 'Barrett狙い'] = float(targets[0])
                df.loc[index, 'EVO狙い'] = float(targets[1])
                df.loc[index, 'Hill狙い'] = float(targets[2])
                df.loc[index, 'HofferQST狙い'] = float(targets[3])
                df.loc[index, 'Kane狙い'] = float(targets[4])
                df.loc[index, 'PearlDGS狙い'] = float(targets[5])
...
# テキストからIOLデータを抽出する関数
   def extract_iol_data(self, text, iolpw):
        lines = text.split('\n')
        data = {}
        keys = None
        ...
        for line in lines:
            ...
            if iolpw in line:
                keys = re.split(r'\s{1,}', line.strip())[1:]
            elif keys:
   ...
   def extract_numbers(self):
        ...
        try:
            targets = self.extract_iol_data(clipboard_text, str(iolpw))
            return targets
        except:
            return None
